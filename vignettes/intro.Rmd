...
Define some variables to be reused later:
...
Then, you can connect R to headless Chromium/Chrome:

```{r connect}
client <- remote$connect()
```

Extract the domains you need^[a domain is a set of **commands** (sent to Chrome), **events** (received from Chrome) and (data) **types**.]:
