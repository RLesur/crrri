...
The goal of `crrri` is to provide a native Chrome Remote Interface in R using the [Chrome Debugging Protocol](https://chromedevtools.github.io/devtools-protocol/). This is a low-level implementation of the protocol.
...
This package is built on top of the [`websocket`](https://github.com/rstudio/websocket) and [`promises`](https://cran.r-project.org/package=promises) packages. The default design of the `crrri` functions is asynchronous: they return promises. However, you can also use `crrri` with callbacks if you prefer. 
...
Assuming that you have configured the `HEADLESS_CHROME` environment variable (see before), you can start headless Chrome:
