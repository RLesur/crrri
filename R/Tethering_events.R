...
#' Await the event Tethering.accepted or create a callback
#' 
#' **Event description**:  
...
#'        Port number that was successfully bound. Accepted values: `~ .res$port` (to refer to the previous result).
#' @param connectionId A character string. 
#'        Connection id to be used. Accepted values: `~ .res$connectionId` (to refer to the previous result).
...
#'        with an element `method` (that is equal to `"accepted"`)
#'        and an element `params` which is a named list.
#'        The `params` list is composed of
#'        the following element(s): `port`, `connectionId`.
#' 
