...
#' Await the event Database.addDatabase or create a callback
#' 
#' **Event description**:  
#' 
#' 
...
#'         Accepted values: `~ .res$database` (to refer to the previous result).
...
#'        with an element `method` (that is equal to `"addDatabase"`)
#'        and an element `params` which is a named list.
#'        The `params` list is composed of
#'        the following element(s): `database`.
#' 
...
#' @export
Database.addDatabase <- function(promise, database = NULL, .callback = NULL) {
