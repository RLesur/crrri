# DO NOT EDIT BY HAND
#' @include send.R
NULL

#' Await the event Profiler.consoleProfileFinished or create a callback
#' 
#' **Event description**:  
#' 
#' 
#' @param promise An asynchronous result object.
#' @param id A character string. 
#'         Accepted values: `~ .res$id` (to refer to the previous result).
#' @param location A Debugger.Location. 
#'        Location of console.profileEnd(). Accepted values: `~ .res$location` (to refer to the previous result).
#' @param profile A Profile. 
#'         Accepted values: `~ .res$profile` (to refer to the previous result).
#' @param title A character string. 
#'        Profile title passed as an argument to console.profile(). Accepted values: `~ .res$title` (to refer to the previous result).
#' @param .callback A callback function taking one argument. The object passed to
#'        this function is the message received from Chrome: this is a named list
#'        with an element `method` (that is equal to `"consoleProfileFinished"`)
#'        and an element `params` which is a named list.
#'        The `params` list is composed of
#'        the following element(s): `id`, `location`, `profile`, `title` (optional) .
#' 
#' @return An async value of class `promise`.
#'         The value and the completion of the promise differ according to the use of a callback function.
#'         When `.callback` is `NULL`, the promise is fulfilled when the event is received.
#'         Its value is a named list of two elements: `ws` (the websocket connexion) and `result`.
#'         `result` is a named list: its elements are the parameters sended by Chrome. 
#'         You can chain this promise with another command or event listener.
#'         When `.callback` is not `NULL`, the promise is fulfilled as soon as the callback is created; the value
#'         is a function without any argument that can be called to cancel the callback. When you use the
#'         `.callback` argument, you cannot send the result to any other command or event listener.
#' @export
Profiler.consoleProfileFinished <- function(promise, id = NULL, location = NULL, profile = NULL, title = NULL, .callback = NULL) {
  method <- 'Profiler.consoleProfileFinished'
  args <- utils::head(rlang::fn_fmls_names(), -1)
  args <- args[!sapply(mget(args), is.null)]
  params <- mget(args)
  params <- if (length(params) > 1) params[2:length(params)] else NULL
  listen(promise, method, params, .callback)
}


#' Await the event Profiler.consoleProfileStarted or create a callback
#' 
#' **Event description**:  
#' Sent when new profile recording is started using console.profile() call.
#' 
#' @param promise An asynchronous result object.
#' @param id A character string. 
#'         Accepted values: `~ .res$id` (to refer to the previous result).
#' @param location A Debugger.Location. 
#'        Location of console.profile(). Accepted values: `~ .res$location` (to refer to the previous result).
#' @param title A character string. 
#'        Profile title passed as an argument to console.profile(). Accepted values: `~ .res$title` (to refer to the previous result).
#' @param .callback A callback function taking one argument. The object passed to
#'        this function is the message received from Chrome: this is a named list
#'        with an element `method` (that is equal to `"consoleProfileStarted"`)
#'        and an element `params` which is a named list.
#'        The `params` list is composed of
#'        the following element(s): `id`, `location`, `title` (optional) .
#' 
#' @return An async value of class `promise`.
#'         The value and the completion of the promise differ according to the use of a callback function.
#'         When `.callback` is `NULL`, the promise is fulfilled when the event is received.
#'         Its value is a named list of two elements: `ws` (the websocket connexion) and `result`.
#'         `result` is a named list: its elements are the parameters sended by Chrome. 
#'         You can chain this promise with another command or event listener.
#'         When `.callback` is not `NULL`, the promise is fulfilled as soon as the callback is created; the value
#'         is a function without any argument that can be called to cancel the callback. When you use the
#'         `.callback` argument, you cannot send the result to any other command or event listener.
#' @export
Profiler.consoleProfileStarted <- function(promise, id = NULL, location = NULL, title = NULL, .callback = NULL) {
  method <- 'Profiler.consoleProfileStarted'
  args <- utils::head(rlang::fn_fmls_names(), -1)
  args <- args[!sapply(mget(args), is.null)]
  params <- mget(args)
  params <- if (length(params) > 1) params[2:length(params)] else NULL
  listen(promise, method, params, .callback)
}
