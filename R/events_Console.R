...
#' Await the event Console.messageAdded or create a callback
#' 
#' **Event description**:  
#' Issued when new console message is added.
#' 
#' @param promise An asynchronous result object.
#' @param message A ConsoleMessage. 
#'        Console message that has been added. Accepted values: `~ .res$message` (to refer to the previous result).
...
#'        with an element `method` (that is equal to `"messageAdded"`)
#'        and an element `params` which is a named list.
#'        The `params` list is composed of
#'        the following element(s): `message`.
#' 
